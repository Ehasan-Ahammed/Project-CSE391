...
$attributes_query = "SELECT * FROM product_attributes WHERE product_id = $product_id ORDER BY id";
    ...
    if (empty($name) || empty($description) || $price <= 0 || $quantity < 0 || empty($category_id)) {
        ...
        if (empty($error)) {
            ...
            if (mysqli_query($conn, $sql)) {
                ...
                // First delete existing attributes if we have new ones
                ...
                if (isset($_POST['delete_images']) && is_array($_POST['delete_images'])) {
                    foreach ($_POST['delete_images'] as $image_id) {
                        ...
                        // Get image filename
                        $img_query = "SELECT image FROM product_images WHERE id = $image_id AND product_id = $product_id";
                        ...
                        if ($img_result && mysqli_num_rows($img_result) > 0) {
                            ...
                            // Delete file
                            $image_path = $upload_dir . $img['image'];
...
<head>
    ...
    <title>Edit Product - Artizo Admin</title>
    <!-- Tailwind CSS -->
...
<body>
    <div class="flex">
        ...
        <div class="ml-64 flex-1 p-8">
            <div class="flex justify-between items-center mb-8">
                <h1 class="text-2xl font-bold">Edit Product: <?php echo htmlspecialchars($product['name']); ?></h1>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" id="name" name="name" class="w-full border rounded-md px-4 py-2" value="<?php echo htmlspecialchars($product['name']); ?>" required>
                            </div>
                            
                            <div class="mb-4">
                                <label for="description" class="block text-sm font-medium text-gray-700 mb-1">Description <span class="text-red-600">*</span></label>
                                <textarea id="description" name="description" rows="5" class="w-full border rounded-md px-4 py-2" required><?php echo htmlspecialchars($product['description']); ?></textarea>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4 mb-4">
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="number" id="price" name="price" step="0.01" min="0" class="w-full border rounded-md pl-8 pr-4 py-2" value="<?php echo $product['price']; ?>" required>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        <span class="absolute left-3 top-2">$</span>
                                        <input type="number" id="sale_price" name="sale_price" step="0.01" min="0" class="w-full border rounded-md pl-8 pr-4 py-2" value="<?php echo $product['sale_price']; ?>">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <input type="number" id="quantity" name="quantity" min="0" class="w-full border rounded-md px-4 py-2" value="<?php echo $product['quantity']; ?>" required>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <div class="flex space-x-4">
                                    <label class="inline-flex items-center">
                                        <input type="radio" name="status" value="active" class="form-radio" <?php echo ($product['status'] == 'active') ? 'checked' : ''; ?>>
                                        <span class="ml-2">Active</span>
                                    </label>
                                    <label class="inline-flex items-center">
                                        <input type="radio" name="status" value="inactive" class="form-radio" <?php echo ($product['status'] == 'inactive') ? 'checked' : ''; ?>>
                            ...
                            <div class="mb-4">
                                <label class="inline-flex items-center">
                                    <input type="checkbox" name="featured" value="1" class="form-checkbox" <?php echo ($product['featured'] == 1) ? 'checked' : ''; ?>>
                        ...
                        <div>
                            ...
                            <div class="mb-4">
                                ...
                                <?php if (!empty($product['image'])): ?>
                                    <div class="mb-2">
                                        <img src="../assets/images/products/<?php echo $product['image']; ?>" alt="<?php echo $product['name']; ?>" class="h-24 w-24 object-cover rounded">
                                    </div>
                                <?php endif; ?>
                                <input type="file" id="image" name="image" class="w-full border rounded-md px-4 py-2" accept="image/*">
                                <p class="text-xs text-gray-500 mt-1">Leave empty to keep current image. Recommended size: 800x800px. Max file size: 2MB</p>
                            </div>
                                ...
                                <?php if (mysqli_num_rows($images_result) > 0): ?>
                                    <div class="grid grid-cols-4 gap-2 mb-3">
                                        <?php while ($image = mysqli_fetch_assoc($images_result)): ?>
                                            <div class="relative">
                                                <img src="../assets/images/products/<?php echo $image['image']; ?>" alt="" class="h-20 w-20 object-cover rounded">
                                                <div class="mt-1">
                                                    <label class="inline-flex items-center text-xs">
                                                        <input type="checkbox" name="delete_images[]" value="<?php echo $image['id']; ?>" class="form-checkbox">
                                ...
                                <?php if (mysqli_num_rows($attributes_result) > 0): ?>
                                    <?php while ($attr = mysqli_fetch_assoc($attributes_result)): ?>
                                        <div class="attribute-row grid grid-cols-4 gap-2 mb-3">
                                            <div>
                                                <input type="text" name="attribute_names[]" placeholder="Name (e.g. Color)" class="w-full border rounded-md px-3 py-2 text-sm" value="<?php echo htmlspecialchars($attr['attribute_name']); ?>">
                                            </div>
                                            <div>
                                                <input type="text" name="attribute_values[]" placeholder="Value (e.g. Red)" class="w-full border rounded-md px-3 py-2 text-sm" value="<?php echo htmlspecialchars($attr['attribute_value']); ?>">
                                            </div>
                                            <div>
                                                <input type="number" name="attribute_prices[]" placeholder="Price Adj." step="0.01" class="w-full border rounded-md px-3 py-2 text-sm" value="<?php echo $attr['price_adjustment']; ?>">
                                            </div>
                                            <div class="flex">
                                                <input type="number" name="attribute_quantities[]" placeholder="Qty" min="0" class="w-full border rounded-md px-3 py-2 text-sm" value="<?php echo $attr['quantity']; ?>">
    ...
    <script>
        ...
        // Add remove event listeners to existing attribute rows
